// Animasi section saat muncul di viewport
...
// Kurangi jumlah partikel jadi 40
for (let i = 0; i < 40; i++) {
  ...
  particles.push({
    ...
    x: 0, y: 0,
    r: 1 + depth * 3,
    dx: (Math.random() - 0.5) * (0.1 + depth * 0.5),
    dy: (Math.random() - 0.5) * (0.1 + depth * 0.5),
    blur: 2 + depth * 6, // blur lebih kecil → ringan
...
function animate() {
  ...
  particles.forEach(p => {
    ...
    if (nodeX && nodeY) {
      ...
      if (dist < 100) { // radius lebih kecil → ringan
        p.baseX -= dx*0.004*p.depth; 
        p.baseY -= dy*0.004*p.depth; 
      }
    }
  });
  ...
  for (let i=0;i<particles.length;i++){
    for (let j=i+1;j<particles.length;j++){
      ...
      const dist=Math.sqrt(dx*dx+dy*dy);
      if (dist<100){ // jarak garis lebih kecil
        ctx.beginPath();
